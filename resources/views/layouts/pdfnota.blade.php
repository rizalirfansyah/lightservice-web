    ...
    <table width="100%" style="font-family: sans-serif;" cellpadding="10">
        <tr>
            <td width="49%" style="border: 0.1mm solid #eee;">Nama : {{ $transaksi->pelanggan->nama_pelanggan }}<br>Nomor Servis : {{ $transaksi->repair->nomor_servis }}<br>Alamat : {{ $transaksi->pelanggan->alamat }}<br>No telp : {{ $transaksi->pelanggan->notelp }}<br>
                Tanggal Selesai : {{ $transaksi->created_at }} 
                <br> Terbayar : {{ $tanggalCetak }}
            </td>
            <td width="2%">&nbsp;</td>
    ...
    <table class="items" width="100%" style="font-size: 14px; border-collapse: collapse;" cellpadding="8">
        <thead>
            <tr>
                ...
                <td width="25%" style="text-align: left;"><strong>Gejala</strong></td>
                <td width="45%" style="text-align: left;"><strong>Detail Servis</strong></td>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="padding: 0px 5px; line-height: 20px;">
                    @foreach($detailServis->where('perbaikan_servis_id', $transaksi->repair->id) as $detail)
                    {{ $detail->tanggal_input }} {{ $detail->kerusakan_servis }} <br>
                    @endforeach
                </td>
